    ...
    public function send(Request $request)
    {
        ...
        try{
            ...
            if (!$message->wasRecentlyCreated) {
                ...
                return response()->json(['code' => -1, 'message' => '发送申请失败']);
            ...
            $table = $server->wsTable->get('uid:' . $message->to_user_id);

            if($table && isset($table['value']) && $server->isEstablished($fd = $table['value'])) {

                $server->push($fd, json_encode(['type' => 'message', 'data' => ['message' => '收到来自'.$user->username.'的消息']]));

            }

            return response()->json(['code' => 0, 'message' => '发送申请成功', 'data' => []]);
