...
;layui.define(["layer", "laytpl", "upload"], function (i) {
    ...
    var x, b, w, k, C, S = function (i, a, n) {
        ...
        }, addGroup: function () {//TODO 新增添加分组逻辑

            layui.use('jquery', function (JQ) {

                let content = '<div class="layui-layer-content" style="text-align: center;padding: 10px;">\n' +
                    '            <input type="text" id="LAY_layimAddGroup" name="group" autocomplete="off" class="layui-input">\n' +
                    '        </div>';
                return t.close(ti.find.index), ti.find.index = t.open({
                    ...
                    content: content,
                    yes:function (e, t) {
                        let content = t.find("#LAY_layimAddGroup").val();
                        if (content === '' || JQ.trim(content).length === 0) {
                            layer.tips('请输入分组名', '#LAY_layimAddGroup', {tips: [3, '#3595cc'], anim: 4});
                            return false;
                        }
                        //发送ajax
                        JQ.ajaxSetup({
